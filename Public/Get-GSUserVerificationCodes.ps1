...
$URI = "https://www.googleapis.com/admin/directory/v1/users/$User/verificationCodes"
try
    {
    ...
    if (!$Raw -and $response.items)
        ...
        if ($CodesOnly)
            {
            $response = $response | Select-Object -ExpandProperty verificationCode
            }
        }
    elseif (!$response.items)
        {
        Write-Warning "No Verification Codes found for user $User!"
        return
        }
    }
catch
