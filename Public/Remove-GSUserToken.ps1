...
Process
    {
    $URI = "https://www.googleapis.com/admin/directory/v1/users/$User/tokens/$ClientID"
    if ($PSCmdlet.ShouldProcess("Removing token for Client ID '$ClientID' from $User"))
        {
        try
            {
            $response = Invoke-RestMethod -Method Delete -Uri $URI -Headers $header -ContentType "application/json"
            if (!$response){Write-Host "Token for Client ID '$ClientID' successfully removed from $User"}
            }
        catch
            {
            try
