...
Begin
    ...
    $URI = "https://www.googleapis.com/admin/directory/v1/users/$User/verificationCodes/invalidate"
    $User = $User
    }
Process
    {
    try
        ...
        if (!$response){Write-Host "Verification codes successfully revoked for $User"}
        }
    catch
        {
        try
