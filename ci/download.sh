...
case "$OSTYPE" in
    linux*)
        ...
        case "$ID" in
            centos*)
                ...
                package=powershell-6.0.2-1.rhel.7.x86_64.rpm
                ;;
            ubuntu)
                if ! hash curl 2>/dev/null; then
                    echo "curl not found, installing..."
                ...
                case "$VERSION_ID" in
                    14.04)
                        package=powershell_6.0.2-1.ubuntu.14.04_amd64.deb
                        ;;
                    16.04)
                        package=powershell_6.0.2-1.ubuntu.16.04_amd64.deb
                        ;;
                    17.04)
                        package=powershell_6.0.2-1.ubuntu.17.04_amd64.deb
                        ;;
                    *)
    ...
    darwin*)
        ...
        package=powershell-6.0.2-osx.10.12-x64.pkg
        ;;
    *)
